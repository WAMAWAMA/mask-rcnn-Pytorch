...
from .layers import ClassifierHead, ConvBnAct, DropPath, create_attn, get_norm_act_layer, AAConvBnAct
from .registry import register_model
import math

__all__ = ['AACspNet']  # model_registry will add each entrypoint fn to this
...
class AABottleneck(nn.Module):
    ...
    def __init__(self, in_chs, out_chs, dilation=1, bottle_ratio=0.25, groups=1,
        ...
        self.conv1 = AAConvBnAct(in_chs, out_chs, kernel_size=3, dk=k * out_chs, dv=int(v * out_chs), Nh=Nh, relative=True, shape=original_shape)
        ...
        if stride != 1 or in_chs != out_chs:
            self.shortcut = nn.Sequential(
                AAConvBnAct(in_chs, out_chs, kernel_size=3, dk=k * out_chs, dv=int(v * out_chs), Nh=Nh, relative=True, stride=stride, shape=shape),
            )
...
class AACrossStage(nn.Module):
    ...
    def __init__(self, in_chs, out_chs, stride, dilation, depth, block_ratio=1., bottle_ratio=1., exp_ratio=1.,
                 ...
                 block_fn=AABottleneck, **block_kwargs):
        super(AACrossStage, self).__init__()
...
class AACspNet(nn.Module):
    ...
    def __init__(self, cfg, in_chans=3, num_classes=1000, output_stride=32, global_pool='avg', drop_rate=0.,
                 ...
                 zero_init_last_bn=True, stage_fn=AACrossStage, block_fn=AABottleneck):
        super().__init__()
...
def _create_aacspnet(variant, pretrained=False, **kwargs):
    cfg_variant = variant.split('_')[0]
    return build_model_with_cfg(
        AACspNet, variant, pretrained, default_cfg=default_cfgs[variant],
        feature_cfg=dict(flatten_sequential=True), model_cfg=model_cfgs[cfg_variant], **kwargs)


@register_model
def AAcspresnet50(pretrained=False, **kwargs):
    return _create_aacspnet('AAcspresnet50', pretrained=pretrained, **kwargs)
